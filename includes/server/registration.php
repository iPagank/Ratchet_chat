...
function register_check($name, $password){
    ...
    $valid_name = "/^[A-Z]{1}+[a-zA-Z0-9._-]{1,}/";
    $valid_password = "/^[A-Z]{1}+[a-zA-Z0-9._-]{1,}/";
    if($name && $password){
            if(preg_match($valid_name,$name) == 1 && preg_match($valid_password,$password) == 1){
            ...
            Password must: Begin with a capital letter, contain at least two characters, at least one number');
            echo json_encode($result);
